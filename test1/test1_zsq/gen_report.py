...
IMG_METRICS = os.path.join(os.path.dirname(__file__), 'output', 'metrics_bar.png')
...
for t in [
    ...
    '⑥ 多维评估：classification_report 输出 Precision/Recall/F1、指标分组柱状图可视化、混淆矩阵、ROC 曲线 + AUC；',
...
add_para(c8,
    ...
    'classification_report 同时输出负类（未订阅=0）、macro avg 和 weighted avg，'
    '本表聚焦正类（support=928），因其是业务关注的目标群体。',
    indent_first=True)
add_para(c8, '')
build_table(c8,
...
add_para(c8,
    '混淆矩阵数值验证了柱状图的定性结论。'
    'SMOTE：TN=6725, FP=585, FN=176, TP=752；FP 最少（误报率 8.0%），FN=176（漏报率 19.0%）；'
    'UnderSample：FN 降至 65（漏报率 7.0%），但 FP 升至 1,096（误报率 15.0%）；'
    'ClassWeight：FN=56（漏报率最低 6.0%），FP=1,096 与欠采样相当。'
    '从业务角度：每减少 1 个漏报（FN），可能需要增加约 5.4 个误报（FP），这是 Precision-Recall 内在权衡的体现。',
    indent_first=True)
...
add_para(c8,
    '三条 ROC 曲线均高度贴近左上角，AUC 在 0.9500~0.9520 之间（差异仅 0.0020），'
    '说明三种策略在阈值无关的整体排序能力上几乎一致，与混淆矩阵所呈现的"决策阈值层面差异"形成对比：'
    'ROC-AUC 衡量的是"模型能否给正样本打出更高概率"，与最终分类阈值无关；'
    '而混淆矩阵中的 FP/FN 差异是在默认阈值 0.5 下的截面表现，可通过调整阈值在 ROC 曲线上滑动改变。'
